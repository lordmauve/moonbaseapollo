 ...
 *
 * $Id: fastrng.c 180 2009-03-15 14:36:12Z casey.duncan $
 */
...
*/
static unsigned int z=362436069, w=521288629;
...
static unsigned int kn[128], ke[256];
...
{
    /*
        unsigned int r = ((unsigned int) rand()) << 1L;
        r |= rand() & 1L;
        return r; */
...
rand_uni(void)
{
/*        return (float) rand_int32() / (float) (2 << 32);  */
...
rand_norm(const float mu, const float sigma)
{
	int hz = rand_int32();
	int iz = hz & 127;
	return mu + (((unsigned int)labs(hz) < kn[iz]) ? hz * wn[iz] : norm_outlier(hz, iz)) * sigma;
}
...
*/
static float
expo_outlier(unsigned int hz, unsigned int iz)
{
	float x;
...
{
	unsigned int hz = rand_int32();
	unsigned int iz = hz & 255;
