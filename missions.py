...
STATION_POS = v(-3000, 1600)
m = Mission('Update ship')
m.say("{control}: Good news, {name}!")
m.spawn('objects.SpaceDock', STATION_POS)
m.fail_if_object_destroyed('lugger')
m.say("{control}: A ship ugprade just arrived at space dock.")
m.spawn('objects.Lugger', STATION_POS + v(30, -35), signpost='Lugger 1', rotation=180, persistent=False, id='lugger')
m.say("{control}: Go get it then!", delay=0)
m.goal('Collect new ship')
m.say_if_region_entered(STATION_POS, 300, 'Manouver {name} to dock with Lugger 1.', colour=WHITE)
